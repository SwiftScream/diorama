...
struct DioramaRandomTests {
    ...
    func `record returns a known source sequence through shared references`() async throws {
        ...
        let instance = try DioramaRandomSystem.instance(named: key.rawValue) {
            probe.makeSource()
        }
    ...
    func `passthrough uses only its live source`() async throws {
        ...
        let instance = try DioramaRandomSystem.instance(named: key.rawValue) { probe.makeSource() }
    ...
    func `named random systems retain independent sources`() async throws {
        ...
        let firstInstance = try DioramaRandomSystem.instance(named: first.rawValue) { firstProbe.makeSource() }
        let secondInstance = try DioramaRandomSystem.instance(named: second.rawValue) { secondProbe.makeSource() }
    ...
    private func attachment(key: AttachmentKey, values: [UInt64]) throws -> ScenarioAttachment {
        let layout = try DioramaRandomSystem.instance(named: key.rawValue).attachment

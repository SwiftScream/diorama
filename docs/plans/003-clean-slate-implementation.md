...
| [Overview](../design-overview.md)                                                            | Ignoring a configured attachment changes verification only. Every loaded payload still requires persistent registration, preparation, and schema validation before unmatched attachments are diagnosed and discarded. 003-C01–003-C06 including 003-C04A, 003-J03.                                                                                                                         |
...
- Status: Complete; owner confirmed scope and GPT-5.6 Sol at `high` reasoning on 2026-09-07.
...
- Expected files/modules: `Mintfile`, `.swiftformat`, `.swiftlint.yml`, `scripts/{format,lint,test,check}`, coverage helper, `.github/workflows/`, `.github/dependabot.yml`, `.codecov.yml`, internal developer setup instructions in `README.md`, and [003-A04 bootstrap evidence](../evidence/003-A04-quality-and-ci-bootstrap.md).
...
- Status: Complete; owner confirmed scope and GPT-6 Astra at `high` reasoning on 2026-09-09, then accepted the review unit on 2026-09-12.
...
- Expected files/modules: `DioramaCore` diagnostics/reporter, preparation boundary, and health types; consumer transform and failing-sink tests; [003-B02 evidence](../evidence/003-B02-prepared-admission-and-diagnostics.md).
...
- Tests/verification: V-code; repeated/concurrent/canceled finish, cleanup failure continuing later cleanup, unused and ignored facts, safe deterministic reports, unchanged repeated results after late misuse, diagnostics racing result freeze without loss or duplication, inspection after execution release, reporter release when its remaining owners release it. The owner-approved 2026-09-18 prerequisite revision removes unattached inventory from core; C04 diagnoses and discards unmatched loaded attachments.
...
- Public behavior: Any replay requires a usable programmatic or loaded baseline before activation.
  ...
  Successfully loaded attachments absent from current setup are diagnosed and discarded from execution and future candidate construction.
- Tests/verification: V-code; missing versus empty, malformed/unsupported data, missing registration including ignored and unmatched systems, unmatched-attachment discard, mixed-mode refusal, zero adapter activation/live access on bad replay setup, untouched old destination.
...
- Tests/verification: V-code; clock replay with live HTTP capture, timed location with HTTP timeout, independent random domains, full file re-record/replay, partial mixed-mode preservation, ignored payloads and unmatched-attachment discard, redaction failures blocking whole publication, concurrent finish and no late replay callbacks, cross-bridge resources/schema goldens, all platform coverage uploads.
